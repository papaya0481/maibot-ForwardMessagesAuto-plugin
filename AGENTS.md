...
- 新增或修改任何由插件交给 Planner 的上下文内容或结构前，必须先把完整文本及注入位置交由用户人工复核；未经用户明确确认不得落地。范围包括但不限于 Prompt、system reminder、Tool 名称、描述与参数说明、主动任务的 intent、reason 和 metadata、Hook 注入或改写文本、合成消息，以及 Planner 可见的 ToolResult、拒绝和错误提示。
- 已经人工复核通过的现有内容不得自行改动，除非用户再次明确许可。
...
- 任何会改变插件版本的修改，都必须在 `dev` 分支同步维护根目录下的
  `CHANGELOG.md`，写明新增、变更、修复、弃用或移除的内容。Changelog 应遵循
  主流开发规范，保留 `Unreleased` 小节，并按版本号和发布日期组织历史记录。
  这一要求只适用于 `dev` 的发布源树及指向其发布 commit 的版本 tag；
  `public-release` 和 `main` 按设计不得包含 `CHANGELOG.md`，远端代码审查不得将
  其缺失视为遗漏，也不得要求在公开分支补回该文件或 Manifest 的 `changelog`
  字段。
...
2. `maisaka.planner.after_response` 的 EARLY blocking Hook 必须无 I/O 地清洗整批转发调用，只保留公开参数；每个转发调用若携带历史内部凭据，只撤销该凭据并重新签发绑定 Host 提供的真实 Planner session 与 `msg_id` 的一次性凭据，不得影响同 session 的其他并发调用。EARLY 还需生成只沿当前 Hook 分发链传递的随机轮次标记；LATE Hook 只能验证轮次和调用绑定，不能重新签发或改绑。模型提供的 `platform`、`group_id`、`stream_id`、`chat_id`、target 或内部凭据均不可信。
