...
### 3.1 在现有“先查看”路径上增加“直接请求”路径

当前版本已经支持路径 A。未来目标是在保留现有路径的基础上增加路径 B，使
Planner 不必总在请求转发前主动查看完整内容。

#### 路径 A：Planner 已经查看（当前已实现）

Planner 先调用 `view_forward_message`，再请求转发。插件复用当前上下文中已有
的完整结果作为查看资格和分享判断依据，不重复展开或重复进行媒体分析，并沿用
现有的消息归属校验、顺序发送和永久防重机制。

发送时，插件已经通过 `send.forward(return_details=True)` 请求平台最终目标
消息 ID，将取得的 ID 与 `sent` 阶段一并持久化，并通过主动任务 metadata 和
意图文本提供给目标 Planner，作为 `reply` 的唯一锚点。当前缺少的不是目标
消息 ID 的取得或传递，而是把 source 已有的完整展开文本与该目标真实消息建立
结构化、幂等的上下文关联。现有版本不会为此重新注入完整内容；这项关联仍需
等待 Host 提供安全的上游能力。上游能力可用后，路径 A 应直接复用已有结果完成
关联，不得再次查看或重复进行媒体分析。
