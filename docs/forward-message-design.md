...
Planner 可以根据消息预览和当前语境直接请求转发。插件完成物理发送后，由
系统级能力自动展开源合并转发内容；这一步由插件或 Host 固定执行，不再交给
agent 决定是否调用工具。展开结果随后注入目标群上下文，再触发目标群 Planner，
使其能够基于完整内容决定评论或沉默。

该路径的关键顺序为：

```text
校验请求 -> 发送真实消息 -> 系统自动展开 -> 注入目标上下文 -> 触发目标 Planner
```

自动展开或上下文注入失败不能把已经成功发送的消息回滚或重新发送。实现时应
单独记录“已发送、内容准备失败”和“已发送、Planner 触发失败”等阶段，让重试
只补做缺失步骤。展开内容只注入一次，不应与真实消息历史、媒体二进制或已有
查看结果重复占用上下文。
...
建议写入插件数据目录中的 `debug_forward_stats.json`，不要写日志全文、消息
正文、媒体数据、群号或其他不必要的敏感内容。示例结构：
...
或统计逻辑异常时，只记录调试警告并跳过统计，不得影响正常转发。
